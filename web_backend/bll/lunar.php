...
function get_lunar_year_info()
{
    // $lun = new Lunar();
    // return $lun->getLunarYearName(getCurYear());
    $ret = "";
    $lun = new Lunar();
    $larr = $lun->convertSolarToLunar(getCurYear(), getCurMonth(), getCurDay());
    if (count($larr) >= 4) 
    {
        $ret = $larr[3];
    }
...
{
    $lun = new Lunar();
    writeln($lun->getLunarYearName(2021));
    $larr = $lun->convertSolarToLunar(2021, 12, 10);
    writearray($larr);
    $larr = $lun->convertSolarToLunar(2022, 1, 3);
    writearray($larr);
    writeln(get_lunar_year_info());
    writeln(get_lunar_day_info());
}
