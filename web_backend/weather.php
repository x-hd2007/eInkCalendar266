...
{
    $wcache = get_cache_weather("now", $cityid);
    if (strlen($wcache) > 0)
    {
        writeln($wcache);
        return ;
    ...
    $durl = "https://devapi.qweather.com/v7/weather/now?gzip=n&location=";
    $durl = $durl .$cityid;
    $durl = $durl ."&key=" .$wkey ."&lang=zh&unit=m";
    $content = file_get_contents($durl);
    saveWeather($cityid, "now", $content);
    writeln($content);
}
...
function weather_main()
{
    ...
    if (!isset($ak) || !isset($cid) ||
        $ak == "" || $cid == "")
    {
        saveAuthLog($_SERVER['REMOTE_ADDR'], "weather.php", " - 错误的请求参数");
    ...
    {
        saveAuthLog($_SERVER['REMOTE_ADDR'], "weather.php", " - 错误的访问密码");
        $einfo = make_error_500();
        header('Content-Type:application/json; charset=utf-8');
        writeln(my_json_encode($einfo));
        return ;
    ...
    saveAuthLog($_SERVER['REMOTE_ADDR'], "weather.php", "获取当前日天气预报");
    header('Content-Type:application/json; charset=utf-8');
    get_weather($cid);
}
