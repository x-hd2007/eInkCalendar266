...
vector<GeoInfo> QWeather::GetGeoInfoList(String location, String adm, String range, uint8_t maxNumber)
{
    ...
    String encodedURL = QWEATHER_GEOAPI_BASE_URL + "/v2/city/lookup?key=" + _key + "&gzip=n&location=" + urlencode(location) 
                        + "&adm=" + urlencode(adm) + "&range=" + range + "&number=" + String(maxNumber) + "&lang=" + _lang;
    Serial.println(encodedURL);

    if (httpClient.begin(client, encodedURL))
    {
...
CurrentWeather QWeather::GetCurrentWeather(String location)
{
    ...
    requestUrl += g_calendarConfig._server;
    requestUrl += "/todotask/weather.php?ak=hello1234554321&cid=";
    requestUrl += location;
...
CurrentWeather QWeather::GetCurrentWeather(String location)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_WEATHERAPI_BASE_URL_DEV : 
                                                            QWEATHER_WEATHERAPI_BASE_URL_COMMERCIAL) 
                                                            + "/now?gzip=n" + "&location=" + location 
                                                            + "&key=" + _key + "&lang=" + _lang 
                                                            + "&unit=" + ((_unitType == UnitType::METRIC) 
                                                            ? "m" : "i");
...
vector<DailyWeather> QWeather::GetDailyWeather(String location, DailyPredictionType dailyPredictionType)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_WEATHERAPI_BASE_URL_DEV : QWEATHER_WEATHERAPI_BASE_URL_COMMERCIAL) 
                        + "/"+ String(dailyPredictionType) +"d?gzip=n" + "&location=" + location + "&key=" + _key 
                        + "&lang=" + _lang + "&unit=" + ((_unitType == UnitType::METRIC) ? "m" : "i");
    Serial.println(requestUrl);
    if (httpClient.begin(client, requestUrl))
    {
        u8_t httpCode = httpClient.GET();
...
vector<HourlyWeather> QWeather::GetHourlyWeather(String location, HourlyPredictionType hourlyPredictionType)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_WEATHERAPI_BASE_URL_DEV : QWEATHER_WEATHERAPI_BASE_URL_COMMERCIAL) 
                        + "/"+ String(hourlyPredictionType) +"h?gzip=n" + "&location=" + location + "&key=" 
                        + _key + "&lang=" + _lang + "&unit=" + ((_unitType == UnitType::METRIC) ? "m" : "i");
    if (httpClient.begin(client, requestUrl))
    {
        u8_t httpCode = httpClient.GET();
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY)
...
CurrentAirQuality QWeather::GetCurrentAirQuality(String location)
{
    ...
    String requestUrl = ((_apiVersion == APIVersion::DEV) ? QWEATHER_AIRAPI_BASE_URL_DEV : QWEATHER_AIRAPI_BASE_URL_COMMERCIAL) 
                        + "/now?gzip=n" +  "&location=" + location + "&key=" + _key + "&lang=" + _lang;
    Serial.println(requestUrl);
    if (httpClient.begin(client, requestUrl))
    {
        u8_t httpCode = httpClient.GET();
